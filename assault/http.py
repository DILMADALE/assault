...
async def worker(name, queue, results):
    """ A function to take unmake request from a queue and perform the work then add results in the result list. """
...
async def distribute_work(url, requests, concurrency, results):
    """ Dividie up the work into batches and collect the final results """
    queue = asyncio.Queue()
    ...
    for i in range(concurrency):
        task = asyncio.create_task(worker(f"worker-{i}", queue, results))
        tasks.append(task)
